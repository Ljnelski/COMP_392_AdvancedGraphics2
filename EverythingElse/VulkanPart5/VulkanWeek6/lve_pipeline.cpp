...
namespace lve
{
	...
	PipelineConfigInfo LvePipeline::defaultPipelineCongfigInfo(uint32_t width, uint32_t height)
	{
		...
		configInfo.rasterizationInfo.depthClampEnable = VK_FALSE; // > depth clamp from 0 to one 0 > x is behind camera
		configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;
		configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
		configInfo.rasterizationInfo.lineWidth = 1.0f;
		configInfo.rasterizationInfo.cullMode = VK_CULL_MODE_NONE; // Backface culling
		configInfo.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE; // Order
